<template>
     <div class="pages">
          <v-card class="overview-cards">
               <h2><v-icon large color="primary">mdi-gold</v-icon> Objective</h2>
          ...
          <v-card v-for="(overview, index) in overviewdata.overviews" :key="index" class="overview-cards">
               ...
               <div v-for="sub in overview.sub" :key="sub.h3" class="overview-cards-sub">
                         <h3 class="overview-cards-sub-title">{{ sub.h3 }}<span v-if="sub.current" class="span-current"><v-icon >mdi-hand-pointing-right</v-icon> Current Work</span></h3> 
                         <p class="overview-cards-sub-title-def"><v-icon small>mdi-calendar-month</v-icon> {{ sub.date }}</p>
